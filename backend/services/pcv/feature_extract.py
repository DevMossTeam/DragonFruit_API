# services/pcv/feature_extract.py
import cv2
import numpy as np
import math
...
def extract_features(segmented_img, mask, pixel_per_cm=102.0, density_g_per_cm3=0.22, scale_factor=1.32):
    ...
    Returns:
      tuple(area_cm2, width_cm, height_cm, weight_est_g, ratio)
    """
    # validasi
    ...
    # note: ratio ~ length/diameter but ensure numeric stability
    # (since diameter = 2*radius_cm)
    return round(area_cm2,4), round(width_cm,3), round(height_cm,3), round(weight_g,3), round(ratio,4)
